"""
Extended performance profile class, and related functions.
...
cost. The extended performance profile shows the additional region [0,1],
defining a similar metric.
...
class ExtProfile(prof.Pdata):
    """
    Store data for extended performance profile.
    """
        ...
        for problem in self.problems:
            ...
            for solver in self.solvers:
                if self.data[solver][problem] == float('inf'):
                    continue
                min_time = min([self.data[s][problem]["time"] for s in
                    self.solvers if s != solver])
